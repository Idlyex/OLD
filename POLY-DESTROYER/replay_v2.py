...
# ═══════════════════════════════════════
def load_models(source="hermes", dur="5m"):
    """Load models from registry. Tries source/dur, then fallbacks."""
    base = Path("training/model_registry")
    candidates = [
        base / source / dur,
        base / "hermes" / "5m",
        base / "hermes" / "15m",
        base / "binance" / "5m",
        base / "latest",           # legacy path
    ]
    md = None
    for c in candidates:
        if (c / "meta.json").exists():
            md = c; break
    if md is None:
        raise FileNotFoundError(f"No meta.json in any of: {[str(c) for c in candidates]}")
    ...
    print(f"  Loaded models from {md}: {list(models.keys())}")
